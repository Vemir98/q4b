...
?>
<tr>
    <td data-th="<?=__('Name')?>" <?if($item->status != Enum_Status::Enabled):?> class="rwd-td0 disable"<?endif?>>
        <?if($item->related_id):?>
            <input type="text" class="table_input q4_required" name="profession_<?=$item->id?>_name" value="<?=$item->name?>">
        <?else:?>
            <input type="text" class="table_input q4_required rwd-td0" name="profession_<?=$item->id?>_name" value="<?=$item->name?>">
        <?endif?>
    </td>
    <td data-th="<?=__('Crafts')?>" <?if($item->status != Enum_Status::Enabled):?> class="rwd-td1 disable"<?endif?>>
        <?if(!empty($crafts)):?>
    ...
    <td data-th="<?=__('Catalog Number')?>" <?if($item->status != Enum_Status::Enabled):?> class="rwd-td2 disable"<?endif?>>
    ...
    <?if($item->status == Enum_Status::Enabled):?>
        <td class="rwd-td3 hidden_status" data-th="<?=__('Status')?>">
            <div class="q4_radio">
