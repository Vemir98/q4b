...
<script>
    $(document).ready(function(){
        ...
        var placeInput = $('input.plan-place-cnumber');
        var floorsSelect = $('select[name=floors]');
        var placeNumberInput = $('input[name="place_number"]');
        var objectSelect = $('select.object_id');
        makeAutocomplete();
            ...
            placeInput.autocomplete({
                ...
                onSelect: function (suggestion) {
                    if(suggestion.data) {
                        ...
                        placeInput.blur();
                        floorsSelect.siblings('.checkbox-list').children('.checkbox-list-row').each(function(){
                            if($(this).find('.checked').length){
                                $(this).click();
                            }
                        });
                        floorsSelect.parents('.form-group').find('.check-all-links').hide();

                        floorsSelect.parents('.multi-select-box').addClass('disabled-input');
                        placeNumberInput.val(suggestion.data);
                    }else{
                        floorsSelect.parents('.multi-select-box').removeClass('disabled-input');
                        placeNumberInput.val('');
                        floorsSelect.parents('.form-group').find('.check-all-links').show();
                    }

                }
            });
        }
        placeInput.on('change focus click',function(){
           if( ! $(this).val()){
               floorsSelect.parents('.multi-select-box').removeClass('disabled-input');
               placeNumberInput.val('');
           }
        ...
        $('.floornum .checkbox-list-row, .floornum .check-all-links').on('click',function(){
            setTimeout(function () {
                floorsSelect.change();
            },350)
        });
