...
class CompilerApiFilter extends BaseCompilerFilter
{
    ...
    public function filterDump(AssetInterface $asset)
    {
        ...
        if (preg_match('/1|yes|on|true/i', ini_get('allow_url_fopen'))) {
            $contextOptions = array('https' => array(
                'method'  => 'POST',
                'header'  => 'Content-Type: application/x-www-form-urlencoded',
                'content' => http_build_query($query),
            ));
            if (null !== $this->timeout) {
                $contextOptions['https']['timeout'] = $this->timeout;
            }
            if ($this->proxy) {
                $contextOptions['https']['proxy'] = $this->proxy;
                $contextOptions['https']['request_fulluri'] = (Boolean) $this->proxyFullUri;
            }
