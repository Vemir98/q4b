Vue.component('public-reserve-materials', {
    template: `
        <div class="row ">
            <div class="col-lg-12">
                    ...
                    <div class="inline-options multiselect-parent rm" v-if="!inProjectMode()">
                        ...
                        <div class="multiselect-row">
                            <div class="multiselect-col">
                              <multiselect :class="{disabled: needSelect() || selectedProjects.lenght < 1}" v-model="selectedCompany" :options="companies" @select="companySelected" :placeholder="selectCompanyTxt" label="name" track-by="id"  :allow-empty="false" :searchable="false" :show-labels="false"></multiselect>
                            </div>
                ...
                <div class="table-pannel-tools ">
                    <div class="input inp_ch">
                        <input type="checkbox" class="terms-checkbox" id="ch1" v-model="selectAll"  @change="toggleSelect" >
                        <label for="ch1"></label>
                    </div>
                ...
                <table class="acceptance">
                    ...
                    <tr>
                        ...
                        <th>{{ thName3 }}</th>
                        <th>{{ thName4 }}</th>
                        <th class="td-200 select-unselect-col">{{moreTxt}}</th>
                    </tr>
                    </thead>
                    ...
                    <tr v-for="item in items" :class="{selected: item.checked}">
                        ...
                        <td>
                            <input v-model="item.size" type="text" @change="edited(item)" class="input-editable w-75" >
                        </td>
                        <td>
                            <div class="more-box">
    ...
    methods: {
        ...
        copyToProject : function () {
            ...
            axios.post(this.copyUrl,JSON.stringify({'ids' : idList, 'company': company, 'projects' : projects, 'type': 'rm'}))
                .then(response => {
                   // this.projects = response.data.items;
            });
        },
